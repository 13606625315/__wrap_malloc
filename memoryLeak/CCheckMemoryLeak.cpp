...
static void parse_func_info(const char *symbol)
{
	...
	int ret = get_funcname(symbol, funcname);
	//printf("ret = %d, funcname = %s\n",ret ,funcname);
    if (TRUE == ret)
    {
    ...
    if (0 != size)
    {
        ...
        for (_it = _malloc_info_map.begin(); _it != _malloc_info_map.end(); ++_it)
        {
            ...
            for (i = 0; i < pstBtInfo->size; ++i)
            {
				...
				int ret = get_libname(pstBtInfo->str_bt[i], libname);
				//printf("ret = %d, pstBtInfo->str_bt[i] = %s libname = %s \n",ret,pstBtInfo->str_bt[i], libname);
                if (TRUE == ret)
                {
                    ///trans lib info
...
//	CCheckMemoryLeakSetDymSymbolDir("memoryLeak");
